...
float *normalizeVector(int *vector, int sizeVector, int** extremes);
...
int main(int argc, char *argv[]){
	...
	float* normalizedVector = (float*) malloc(sizeVector * sizeof(float));

	normalizedVector = normalizeVector(vector, sizeVector, extremes);

	fprintVector(normalizedVector, sizeVector);
...
float *normalizeVector(int *vector, int sizeVector, int** extremes){
	...
	for (int x = 0; x < sizeVector; x++) {
		...
		normalizedVector[x] = (float) (vector[x] - (*extremes)[0]) / ((*extremes)[1] - (*extremes)[0]);
